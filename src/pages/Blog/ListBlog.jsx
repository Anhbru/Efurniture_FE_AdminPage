...
const ListBlog = () => {
    ...
    const fetchBlogs = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/api/v1/blogs/get_all_blogs?page=${currentPage}`
        );
    ...
    const fetchCategoriesAndTags = async () => {
      try {
        const categoriesResponse = await axios.get(
          "http://localhost:8080/api/v1/categories-blog/get-all-categoriesBlog"
        );
        const tagsResponse = await axios.get(
          "http://localhost:8080/api/v1/tags-blog/get-all"
        );
  ...
  const fetchTag = async (tagId) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/v1/tags-blog/${tagId}`
      );
      setTags((prevState) => ({
        ...prevState,
        [tagId]: response.data,
  ...
  const fetchTag2 = async (categoryId) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/v1/categories-blog/${categoryId}`
      );
  ...
  const generatePageNumbers = () => {
    ...
    const lastPageIndex = Math.min(
      totalPages - 1,
      firstPageIndex + maxPagesToShow - 1
    );
  ...
  const handleDeleteBlog = async (blogId) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/v1/blogs/delete_blog/${blogId}`
      );
              ...
              <div className="card-body">
                <div className="row mb-2">
                  <div className="col-sm-5">
                    <a href="/create-blog" className="btn btn-danger mb-2">
                      <i className="mdi mdi-plus-circle me-2" /> Add Blog
                    </a>
                  </div>
                    ...
                    <tbody>
                      {blogs.map((blog) => (
                        <tr key={blog.id}>
                          ...
                          <td>
                            <Link
                              to={`http://localhost:3000/blog-detail/${blog.id}`}
                                    ...
                                    if (categorie) {
                                      return (
                                        <React.Fragment key={categorie.id}>
                                          <a
                                            className="bp__category"
                                            href="blog-left-sidebar.html"
                                          >
                                            {categorie.name}
                                          </a>
                                          {index <
                                            blog.categoryBlogIds.length - 1 &&
                                            " "}
                                        </React.Fragment>
                                      );
                                    } else {
