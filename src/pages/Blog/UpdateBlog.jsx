...
const UpdateBlog = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [blogResponse, categoriesResponse, tagsResponse] =
          await Promise.all([
            axios.get(
              `http://localhost:8080/api/v1/blogs/get-blog-detail/${id}`
            ),
            axios.get(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/api/v1/blogs/update_blog/${id}`,
        formData
      );
