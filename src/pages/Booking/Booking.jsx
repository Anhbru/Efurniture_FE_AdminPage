  ...
  const fetchBookings = async (page) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/v1/booking/all?page=${page}&size=10`
      );
      setBookingsPage({
  ...
  const handleReceiveAndConfirm = async (bookingId) => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:8080/api/v1/booking/receive-booking-request/${bookingId}`,
        {
      ...
      const updatedBookings = bookings.map((booking) =>
        booking.id === bookingId
          ? { ...booking, schedule: selectedDates[bookingId] }
          : booking
      );
  ...
  const handleCancelBooking = async (bookingId) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/v1/booking/cancel-booking/${bookingId}`
      );
      console.log(response.data);
                ...
                <div className="table-responsive">
                    ...
                    <tbody>
                      {bookings.map((booking) => (
                        <tr key={booking.id}>
                          ...
                          <td>
                            {booking.streetAddress}, {booking.wardName},{" "}
                            {booking.districtName}, {booking.provinceName}
                          </td>
                          <td>
                            <h5>
                              <span
                          ...
                          <td>
                            <a
                              href={`/view-project-booking/${booking.id}`}
                            ...
                            {booking.status !== "Cancel" && (
                              <a
                                className="action-icon"
                                onClick={() =>
                                  handleReceiveAndConfirm(booking.id)
                                }
                              >
                  ...
                  <div className="pagination justify-content-end pr-3">
                    <button
                      ...
                      onClick={() =>
                        handlePageChange(bookingsPage.currentPage - 1)
                      }
                      disabled={bookingsPage.currentPage === 0}
                    >
                      Previous
                    </button>
                    <span className="align-self-center mr-2">
                      &nbsp;&nbsp;Page {bookingsPage.currentPage + 1} of{" "}
                    ...
                    <button
                      ...
                      disabled={
                        bookingsPage.currentPage === bookingsPage.totalPages - 1
                      }
                    >
                      Next
                    </button>
