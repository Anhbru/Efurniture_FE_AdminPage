...
const UpdateOrderStatus = () => {
  ...
  const handleUpdateProduct = async (event) => {
    //event.prevenDefault();
    axios
      .put("http://localhost:8080/api/v1/orders/" + id, values)
      .then((res) => {
            ...
            >
              <div className="horizontal-steps-content">
                {statusOrder.map((statusOrderItem, index) => {
                  if (
                    statusOrderItem.code !== "cancelled" &&
                    statusOrderItem.code !== "delivery_canceled"
                  ) {
                    return (
                      <div
                        key={statusOrderItem.id}
                        className={`step-item ${
                          selectedStatus === statusOrderItem.id ? "current" : ""
                        }`}
                      >
                        <span
              ...
              <div
                ...
                style={{
                  width: `${
                    (selectedStatus + 2) * (100 / statusOrder.length)
                  }%`,
                }}
              />
            </div>
            ...
            <div className="card">
              <div className="card-body">
                <h4 className="header-title mb-3">
                  Items from Order #{values.fullName}
                </h4>
                  ...
                  <table className="table mb-0">
                    ...
                    <tbody>
                      ...
                      {values.order_details &&
                        values.order_details.map((item, index) => (
                          <tr key={index}>
                            <td>{item.product.name}</td>
                            <td>{item.quantity}</td>
                            <td>${item.product.price}</td>
            ...
            <div className="card">
              <div className="card-body">
                ...
                <ul className="list-unstyled mb-0">
                  <li>
                    <p className="mb-2">
                      <span className="fw-bold me-2">Payment Type:</span>{" "}
                      {values.payment_method}
                    </p>
                    <p className="mb-2">
                      <span className="fw-bold me-2">Shipping method:</span>{" "}
                      {values.shipping_method}
                    </p>
                    <p className="mb-2">
                      <span className="fw-bold me-2">Shipping Date:</span>{" "}
                      {values.shipping_date}
                    </p>
                    {productDetail && (
                      <p className="mb-0">
                        <span className="fw-bold me-2">Payment status:</span>{" "}
                        {productDetail.paymentStatus.name}
                      </p>
                    )}
                  </li>
                </ul>
