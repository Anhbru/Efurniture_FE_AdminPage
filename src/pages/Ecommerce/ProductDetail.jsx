...
const ProductDetail = () => {
  ...
  const fetchCountOrder = async () => {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/v1/orders/count/${id}`
      );
  ...
  const fetchRevenueProduct = async () => {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/v1/orders/revenue/${id}`
      );
                    ...
                    <div className="col-lg-7">
                      <form className="ps-lg-4">
                        ...
                        <h3 className="mt-0">
                          {productDetail.name}{" "}
                          <Link
                            to={`/update-product/${productDetail.id}`}
                            className="text-muted"
                          >
                            <i className="mdi mdi-square-edit-outline ms-2" />
                          </Link>{" "}
                        </h3>
                        <p className="mb-1">
                          Added Date: {productDetail.createdAt}
                        </p>
                        <p className="font-16">
